...
```{r allee_and_r_code,eval=F,echo=F,cache=T,error=F,message=F,warning=F,fig.cap="Optimal spacing for varying Allee and r values along with different disturbance parameters.\\label{fig:allee_and_r}"}
...
params_to_try <- params_to_try[rep(seq(nrow(params_to_try)), each = 5), ]
...
```{r allee_and_r,eval=T,echo=F,cache=T,error=F,message=F,warning=F,fig.cap="Optimal spacing for varying Allee and r values along with different disturbance parameters.\\label{fig:allee_and_r}"}
...
# Run scripts
num_reserves_vec <- rev(c(rep(2,3),rep(3,3)))#,rep(4,10)))
