...
prob_vector = rep(seq(0.0,0.4,by=0.02),100)
...
p1 <- ggplot(data=model_output) +
   ...
   geom_line(aes(x=disturbance_frequency,y=avg_optimal_spacing,color=as.factor(disturbance_mortality)),size=1.5) + scale_color_viridis(discrete = T,alpha=0.5)   + theme_classic(base_size = 14, base_family = "")  + coord_cartesian(xlim = c(0, 0.4),ylim = c(0, 50)) + theme(plot.title = element_text(size = 12, face = "plain")) +
...
```{r different_objectives_code, echo=FALSE,eval=F,cache=TRUE,error=FALSE,message=FALSE,warning=FALSE}
...
params_list <- read.csv(file='../model_outputs/n_patch_optimal_spacing_vs_disturbance_regime_different_objectives_Fig5.csv',header=T)
#params_list$optimal_spacing <- params_list$optimal_spacing-1
...
disturbance_prob_vec = seq(0,0.02,0.005)#0.0005)
disturbance_size_vec = c(2)
disturbance_magnitude_vec = 0.9#seq(0.7,0.9,0.2)
...
```{r reserve_config_code, echo=FALSE,eval=F,cache=TRUE,error=FALSE,message=FALSE,warning=FALSE}
...
params_list <- expand.grid(fishing_values_vec,r_value_vec,omega_value_vec,dispersal_value_vec,disturbance_prob_vec,disturbance_size_vec,disturbance_magnitude_vec,objective_vec)
params_list <- params_list[rep(seq(nrow(params_list)), each = 20), ]
names(params_list) <- c('fishing_values_vec','r_value_vec','omega_value_vec','dispersal_value_vec','disturbance_prob_vec','disturbance_size_vec','disturbance_magnitude_vec','objective_vec')
...
```{r reserve_config, echo=FALSE,eval=T,cache=TRUE,error=FALSE,message=FALSE,warning=FALSE,fig.cap='Reserve locations for 20 different simulations all using the same parameter values. These are the same parameters as Fig. 5 with a set probability of disturbance of 0.01 (a value on Fig. 5 with a lot of variability) and for the persistence objective. Each R denotes the optimal placement of a reserve for each simulation. The density plot to the right denotes the frequency of locations chosen across all simulations.\\label{reserve_config}'}

optimal_reserves_mat <- read.csv(file='../model_outputs/n_patch_optimal_reserve_config_Fig7.csv',header=T)

layout(matrix(c(1,1,1,1,2,2), nrow = 1, ncol = 6, byrow = TRUE))
par(mar=c(0.5,0.5,0.5,0.5),oma=c(4,4,0,0))
matplot(optimal_reserves_mat,pch='R',cex=1,ylab='Reserve location',xlab='Simulation number',ylim=c(12.5,28.5),xlim=c(0.5,20.5),col='black',las=1)
...
my_dens <- density(as.vector(as.matrix(optimal_reserves_mat)))
